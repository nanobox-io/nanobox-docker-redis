...
if payload[:new_member][:schema][:meta][:disk].to_i < `du -s /data/var/db/redis | awk '{print $1}`.to_i
...
execute "send bulk data to new member" do
  command "tar -cf - /data/var/db/redis | ssh #{payload[:new_member][:local_ip]} tar -xpf -"
end
